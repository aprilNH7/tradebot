...
class BinanceExchange(BaseExchange):
    ...
    def place_order(self, symbol: str, side: OrderSide,
        ...
        result = self.client.create_order(
            symbol, ot, side.value, quantity, price
        )
        log.info(f"Order placed: {side.value} {quantity} {symbol} @ {price or 'market'}")
        return Order(
            id=result["id"],
            symbol=symbol,
            side=side,
    ...
    def get_positions(self) -> list[Position]:
        ...
        positions = []
        for asset, v in bal.items():
            if isinstance(v, dict) and v.get("total", 0) > 0 and asset not in ("USDT", "USD", "BUSD"):
                pair = f"{asset}/USDT"
                try:
                    ...
                    positions.append(Position(
                        ...
                        side=OrderSide.BUY,
                        quantity=v["total"],
                        entry_price=0,  # Binance spot doesn't track entry
